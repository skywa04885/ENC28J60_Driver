...
static volatile uint8_t target_ip[4] = { 192, 168, 2, 21 };
...
void udp_callback(uint16_t port, uint16_t len, uint8_t *data)
{
    switch (port)
    {
        ...
        case 4004:
        {
            const char *message = "FUCK YEAH BITCHES!!!";
            printf("Echo packet: %s\n", data);
            enc28j60_send_udp(&cfg, 4004, strlen(message), message, target_ip, target_mac);
            break;
        }
    }
}
